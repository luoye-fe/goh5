...
vm.$data.paginationConf = {
	...
	setTotalItems: function(){
		// 如果总条数是异步返回的，不要在初始配置中设置 totalItems ，在这获取数据后执行 this.totalItems = .. 
	}
};
*/
...
var Pagination = Vue.extend({
	...
	created: function(){
		...
		if (!this.paginationConf.totalItems && this.paginationConf.setTotalItems) {
			this.paginationConf.setTotalItems(function(){
	...
	watch: {
		'paginationConf': {
			handler: function (val, oldVal) {
				...
				var val = this.paginationConf.totalItems + ' ' + this.paginationConf.currentPage + ' ' + this.paginationConf.itemsPerPage;
				this.getPagination(val, oldVal);
			},
      		deep: true
		}
	}
